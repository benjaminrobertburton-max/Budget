...
- The fictional Chrome demo creates its separate disposable profile automatically. The user needs no new Google account and no manual profile setup; never ask them to sign into Chrome sync. Browser integration currently permits loopback fictional pages only, not real banks. Preserve that boundary while developing the separately gated live pilot. Browser demos need the pinned dependency from `collector/pnpm-lock.yaml`; read `collector/README.md` for commands and demonstrated versus remaining capabilities.
- Version 0.4 adds explicit stopped-test recovery (`recover-test` inspects; `--confirm-cleanup` authorizes only proven owned temporary files) and text-free structural inspection. Recovery never kills personal Chrome and must block on uncertain ownership/processes. An outline is not financial evidence or proof of login/coverage. Live navigation, pilot controls, and account-specific evidence extraction remain gated; read the current handoff checklist before requesting a bank sign-in.
