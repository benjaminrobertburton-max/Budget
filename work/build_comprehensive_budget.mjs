...
budget.getRange("A7:D7").values=[["Wife income (variable — personal safe)","Weekly",60,null]]; budget.getRange("D7").formulas=[["=IF(B7=\"Weekly\",C7,C7*12/52)"]];
...
const planRows=[
 ["Housing","Rent / Pearl Point set-aside", "Weekly",300,null,"Fund on the four Tuesdays after rent is paid; $300 x 4 creates a modest cushion at the current rent"],
...
sources.getRange("A4:E13").values=[
 ["Wells Fargo checking","Income and recurring payments", "Jul 8-Aug 7, 2026", "Weekly $1,343.37 payroll; $75/week automatic Fidelity 401(k) transfer; rent rebuilt at $300 on each of the four Tuesdays after a rent payment; prior $188 weekly Credit Union of Texas car-loan transfers", "The prior $805 UMB category was an assumption and has been removed from savings. Rent paid Sep. 1 uses the existing Wealthfront rent bucket; no new rent deposit is made on that payment Tuesday. The next four Tuesday deposits are $300 each ($1,200 total) for the October payment. Fund the car at the annualized rate ($746.47 x 12 / 52) in a reserve, then pay the exact monthly amount on the 27th. An $85 Tuesday Wells Fargo-to-PayPal cruise transfer begins Sep. 1; $50/week is added for a Wealthfront future-expenses fund. AT&T normal baseline is $65/month; Sep. 16 autopay is $88.59 due to a one-time roaming charge."],
...
title(savings,"A1:D1","Savings & Debt",reserves);
savings.getRange("A2:D2").merge();savings.getRange("A2").values=[["Read this as three questions: Is savings protected? Is next month’s rent funded? Are the priority debts on track?"]];savings.getRange("A2:D2").format={font:{italic:true,color:gray},wrapText:true};
sec(savings,"A4:D4","Savings health");
...
savings.getRange("C5:D9").values=[["Next rent reserve",null],["Rent cushion after 4 deposits",null],["Actual Wealthfront transfer — Sep 2",null],["Target Wells remainder — Sep 2",null],["PayPal promo target this week",null]];
savings.getRange("D5").formulas=[["='Support - Budget Inputs'!E12*4"]];savings.getRange("D6").formulas=[["=D5-'2. Tuesday Review'!D14"]];savings.getRange("D7").formulas=[["='2. Tuesday Review'!E23"]];savings.getRange("D8").formulas=[["='2. Tuesday Review'!E24"]];savings.getRange("D9").formulas=[["='Support - Promo Detail'!F12"]];
savings.getRange("C5:D9").format={fill:pale,borders:{preset:"inside",style:"thin",color:"#D9E5E7"}};savings.getRange("D5:D9").format={fill:"#FFFFFF",font:{bold:true,size:14},numberFormat:usd};savings.getRange("C5:D5").format={fill:mint,font:{bold:true}};savings.getRange("C7:D7").format={fill:yellow,font:{bold:true}};
...
widths(savings,[["A:A",31],["B:B",21],["C:C",31],["D:D",21]]);savings.getRange("A1:D22").format.wrapText=true;savings.getRange("A1:D22").format.font={size:12};savings.getRange("A5:D9").format.rowHeight=29;savings.getRange("A12:D17").format.rowHeight=25;savings.getRange("A20:D22").format.rowHeight=27;savings.showGridLines=false;savings.freezePanes.freezeRows(4);
...
weekly.getRange("A2:F2").merge();weekly.getRange("A2").values=[["Use this on payday. Pay or move the listed amounts, then record only unexpected spending below. Statements are a monthly audit, not a required monthly rebuild."]];weekly.getRange("A2:F2").format={font:{italic:true,color:gray},wrapText:true};
...
title(history,"A1:M1","History — Closed Weeks",audit);
...
history.getRange("H6").values=[[476.6607692307693]];history.getRange("G6").values=[["Data verified — Sep 9"]];history.getRange("M5:M6").values=[["Legacy baseline: no full line-level source archive existed before the Sep. 9 audit. Do not treat this row as source-certified."],["Reviewed Wed Sep. 9 because of the holiday delay. Source-controlled import includes the Citi activity correction."]];history.getRange("G5:G30").dataValidation={rule:{type:"list",values:["In progress","Data verified — Sep 9","Legacy — not source-audited","✓ Completed","Needs review"]}};input(history,"D7:H30");input(history,"G5:G30");input(history,"M5:M30");history.getRange("A5:C30").format.numberFormat="mmm d, yyyy";history.getRange("D5:F30").format.numberFormat=usd;history.getRange("H5:L30").format.numberFormat=usd;history.getRange("L5:L30").conditionalFormats.add("cellIs",{operator:"lessThan",formula:0,format:{fill:red,font:{bold:true,color:"#B91C1C"}}});widths(history,[["A:C",17],["D:F",19],["G:G",24],["H:L",17],["M:M",42]]);history.getRange("A1:M30").format.wrapText=true;history.getRange("A1:M30").format.font={size:12};history.getRange("A5:M30").format.rowHeight=24;history.showGridLines=false;history.freezePanes.freezeRows(4);
...
cash.getRange("A5:E7").values=[[new Date("2026-09-09"),"Wells Fargo checking",1227.78,1343.38,75],[new Date("2026-09-09"),"Wealthfront savings",11847.64,0,0],[new Date("2026-09-09"),"Personal safe cash",60,0,0]];cash.getRange("F5:F7").values=[[1227.78],[11847.64],[60]];cash.getRange("G5:G7").values=[["Available balance from Sep. 9 screenshot; posted balance was −$40.60 before pending payroll and Fidelity activity"],["Sep. 9 screenshot; no activity after the Sep. 2 Wells transfer is visible"],["Wife’s income held as cash; excluded from Wells/Wealthfront funding"]];
...
funding.getRange("E5:H12").values=[
 ["Wealthfront rent bucket","Wealthfront to rent payment","Rent-payment Tuesday: $0; then $300 x 4","Sep. 1 rent payment is $1,169.46 from prior contributions. No new contribution today; fund Sep. 8, 15, 22, and 29 for the October payment"],
...
tuesday.getRange("G9:G19").values=[["Pay the current balance if maintaining the weekly payoff routine"],["Pay the current balance if maintaining the weekly payoff routine"],["No payment needed"],["Five weekly payments remain before Oct. 10"],["Move to Wealthfront rent reserve"],["Move to CUTX reserve; do not pay the loan early"],["Do not count again in cash required"],["Included in cash required; confirm after it posts"],["Included in cash required; confirm after it posts"],["Included in cash required; confirm after it posts"],["Transfer this amount from Wealthfront so Wells retains $1 after all actions"]];
...
dash.getRange("A5:C8").values=[["Wells available now",null,"After pending Fidelity debit"],["Cash required today",null,"Including scheduled PayPal savings transfers"],["Move from Wealthfront",null,"Covers the gap while retaining $1 in Wells"],["Wells left after plan",null,"This must match Tuesday Review"]];
