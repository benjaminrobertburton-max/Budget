...
tuesday.getRange("A2:G2").merge();tuesday.getRange("A2").values=[["Use this checklist after Start is verified. For a manual action, use the small dropdown to select ☑ when complete. Automatic and not-due rows need no action."]];tuesday.getRange("A2:G2").format={font:{italic:true,color:gray},wrapText:true};
...
tuesday.getRange("A8:G8").values=[["Type","Payment or transfer","Basis","Planned amount","Status","Action","Confirmation note"]];headers(tuesday,"A8:G8");
