...
sources.getRange("A4:E13").values=[
 ["Wells Fargo checking","Sep. 9 balance + operating history", "Sep 9, 2026", "Available Wells $1,227.78 after pending Fidelity debit; personal-safe balance $220 includes the earlier $60 plus an additional $160", "The earlier Jul–Aug account history establishes the $1,343.37 recurring-payroll baseline and $75 weekly Fidelity transfer. Personal-safe cash is savings-only and excluded from Wells cash-plan and spending-capacity calculations. The Sep. 9 Wells balance controls this live review."],
...
savings.getRange("A12:D16").values=[["Item","Weekly amount","Weeks confirmed","Saved since tracker began"],["Fidelity Roth IRA",75,0,null],["PayPal cruise fund",85,0,null],["Del Mar tuition savings",193.36,0,null],["Future-expenses fund",50,0,null]];headers(savings,"A12:D12");savings.getRange("D13").formulas=[["=B13*C13"]];savings.getRange("D13:D16").fillDown();input(savings,"B13:C16");savings.getRange("B13:B16").format.numberFormat=usd;savings.getRange("D13:D16").format.numberFormat=usd;
...
cash.getRange("A5:E7").values=[[new Date("2026-09-15"),"Wells Fargo checking",1392.72,1343.36,0],[new Date("2026-09-15"),"Wealthfront savings",12026.13,0,0],[new Date("2026-09-15"),"Personal safe cash",510,0,0]];cash.getRange("F5:F7").values=[[1392.72],[12026.13],[510]];cash.getRange("G5:G7").values=[["Available balance from Sep. 15 screenshot. It includes pending Kiewit payroll of $1,343.36; the prior-cycle rent movements netted to $178.49."],["Sep. 15 balance and activity captured; latest visible activity is the Sep. 14 $300 Wells Fargo IFI transfer."],["Current user-reported personal-safe cash balance. Savings-only and excluded from Wells/Wealthfront funding and spending-capacity calculations."]];
