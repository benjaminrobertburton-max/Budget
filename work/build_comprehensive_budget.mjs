...
tuesday.getRange("A6:G6").values=[[null,"Sep 1–Sep 7 purchases","", "Ready to rebuild",null,null,null]];
...
tuesday.getRange("A9:C21").values=[["Card payment","Chase Sapphire","Current Sep. 9 balance"],["Card payment","Citi card","Current Sep. 9 balance"],["Card payment","Discover card","Current balance $0.00"],["Card payment","Prime Visa","Current balance $0.00"],["Card payment","Capital One","No balance due"],["Debt payment","PayPal promotional payoff","Stage 1 payoff target"],["Transfer","Rent fund — October","Actual Sep. 9 funding retained"],["Transfer","Car fund / CUTX","Weekly reserve toward Sep. 27 due date"],["Automatic","Fidelity Roth IRA","Pending; already in Wells available balance"],["Automatic","PayPal cruise savings","Flexible savings contribution"],["Automatic","PayPal tuition savings","Weekly scheduled transfer"],["Automatic","PayPal future-expenses savings","Flexible savings contribution"],["Transfer","Wealthfront to Wells","Cover a remaining cash shortfall"]];
tuesday.getRange("D9:D11").values=[[266.96],[132.09],[0]];tuesday.getRange("D12:D13").values=[[0],[0]];tuesday.getRange("D14").formulas=[["='Support - Promo Detail'!F12"]];tuesday.getRange("D15").values=[[178.49169230769235]];tuesday.getRange("D16").formulas=[["='Support - Budget Inputs'!E13"]];tuesday.getRange("D17").values=[[75]];tuesday.getRange("D18").values=[[85]];tuesday.getRange("D19").values=[[193.36]];tuesday.getRange("D20").values=[[50]];tuesday.getRange("D21").formulas=[["=MAX(0,E23+E25-E24)"]];
tuesday.getRange("E9:E21").values=[["☑ Done"],["☑ Done"],["— Not due"],["— Not due"],["— Not due"],["☑ Done"],["☑ Done"],["☑ Done"],["Auto — no click"],["Auto — no click"],["Auto — no click"],["Auto — no click"],["— Not needed"]];
tuesday.getRange("E9:E21").dataValidation={rule:{type:"list",values:["☐ Pay now","☑ Done","Partial","— Not due","Auto — no click","— Not needed"]}};
tuesday.getRange("F9:F21").values=[["Manual"],["Manual"],["Not due"],["Not due"],["Not due"],["Manual"],["Manual"],["Manual"],["Automatic"],["Automatic"],["Automatic"],["Automatic"],["Not needed"]];
tuesday.getRange("G9:G20").values=[["Pay the current balance if maintaining the weekly payoff routine"],["Pay the current balance if maintaining the weekly payoff routine"],["No payment needed; $0 balance and no pending charges confirmed"],["No payment needed; $0 balance and no pending charges confirmed"],["No payment needed"],["Five weekly payments remain before Oct. 10"],["Actual Sep. 9 transfer retained; do not revise this completed funding amount"],["Move to CUTX reserve; do not pay the loan early"],["Do not count again in cash required"],["Automatic commitment; included in cash required"],["Automatic commitment; included in cash required"],["Automatic commitment; included in cash required"]];tuesday.getRange("G21").formulas=[["=IF(D21=0,\"No transfer needed after net rent funding\",\"Additional funding required after adding Discover\")"]];
...
tuesday.getRange("A22:C22").merge();tuesday.getRange("A22").values=[["Planned subtotal (cash actions)"]];tuesday.getRange("A22:C22").format={fill:pale,font:{bold:true,color:teal}};tuesday.getRange("D22").formulas=[["=SUM(D9:D16,D18:D20)"]];tuesday.getRange("D22").format={fill:pale,font:{bold:true,color:teal},numberFormat:usd,borders:{preset:"doubleBottom",style:"medium",color:teal}};
tuesday.getRange("A23:D23").merge();tuesday.getRange("A23").values=[["Cash required for payments and transfers"]];tuesday.getRange("A23:D23").format={fill:mint,font:{bold:true,color:teal}};tuesday.getRange("E23").formulas=[["=SUM(D9:D16,D18:D20)"]];
tuesday.getRange("A24:D24").merge();tuesday.getRange("A24").values=[["Wells available now after pending Fidelity"]];tuesday.getRange("A24:D24").format={fill:mint,font:{bold:true,color:teal}};tuesday.getRange("E24").values=[[1227.78]];
...
dash.getRange("A5:C8").values=[["Wells available now",null,"Includes pending Kiewit payroll of $1,343.36"],["Wells import","Verified","Anchor and eight new cash actions reconciled"],["Cash plan","Ready","All current required source checkpoints are verified"],["Next action","Build plan","Rebuild Tuesday Review from this week’s verified actions"]];
