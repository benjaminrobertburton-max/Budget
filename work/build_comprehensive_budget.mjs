...
tuesday.getRange("A2:G2").merge();tuesday.getRange("A2").values=[["Use this checklist after Start is verified. For a manual action, change the empty box to a checked box when complete. Automatic and not-due rows need no action."]];tuesday.getRange("A2:G2").format={font:{italic:true,color:gray},wrapText:true};
...
tuesday.getRange("A6").formulas=[["=COUNTIF('1. Start'!E6:E13,\"Verified\")&\" of \"&(COUNTA('1. Start'!F6:F14)-COUNTIF('1. Start'!E6:E14,\"Not due\"))&\" source sets verified\""]];tuesday.getRange("C6").formulas=[["=COUNTIFS(F9:F21,\"Manual\",E9:E21,\"☑\")&\" / \"&COUNTIF(F9:F21,\"Manual\")&\" manual payments complete\""]];
tuesday.getRange("A6:G6").format={fill:pale,font:{bold:true},wrapText:true};
tuesday.getRange("A8:G8").values=[["Type","Payment or transfer","Basis","Planned amount","Done?","Action","Confirmation note"]];headers(tuesday,"A8:G8");
tuesday.getRange("A9:C21").values=[["Card payment","Chase Sapphire","Current Sep. 15 balance"],["Card payment","Citi card","Current Sep. 15 balance"],["Card payment","Discover card","Current balance $0.00"],["Card payment","Prime Visa","Current balance $0.00"],["Card payment","Capital One","No balance due"],["Debt payment","PayPal promotional payoff","Stage 1 payoff target"],["Transfer","Rent fund — October","Only verified Wells capacity after required actions"],["Transfer","Car fund / CUTX","Weekly reserve toward Sep. 27 due date"],["Automatic","Fidelity Roth IRA","Weekly automatic transfer"],["Automatic","PayPal cruise savings","Weekly automatic transfer"],["Automatic","PayPal tuition savings","Weekly automatic transfer"],["Automatic","PayPal future-expenses savings","Weekly automatic transfer"],["Transfer","Wealthfront to Wells","Cover the verified cash shortfall and leave $1 in Wells"]];
tuesday.getRange("D9:D11").values=[[786.38],[106.26],[0]];tuesday.getRange("D12:D13").values=[[0],[0]];tuesday.getRange("D14").formulas=[["='Support - Promo Detail'!F12"]];tuesday.getRange("D15").formulas=[["=MAX(0,E24-SUM(D9:D14,D16:D20)-E25)"]];tuesday.getRange("D16").formulas=[["='Support - Budget Inputs'!E13"]];tuesday.getRange("D17").values=[[75]];tuesday.getRange("D18").values=[[85]];tuesday.getRange("D19").values=[[193.36]];tuesday.getRange("D20").values=[[50]];tuesday.getRange("D21").formulas=[["=MAX(0,E23+E25-E24)"]];
tuesday.getRange("E9:E21").values=[["☐"],["☐"],["Not due"],["Not due"],["Not due"],["☐"],["Not needed"],["☐"],["Automatic"],["Automatic"],["Automatic"],["Automatic"],["☐"]];
tuesday.getRange("E9").dataValidation={rule:{type:"list",values:["☐","☑"]}};tuesday.getRange("E10").dataValidation={rule:{type:"list",values:["☐","☑"]}};tuesday.getRange("E14").dataValidation={rule:{type:"list",values:["☐","☑"]}};tuesday.getRange("E16").dataValidation={rule:{type:"list",values:["☐","☑"]}};tuesday.getRange("E21").dataValidation={rule:{type:"list",values:["☐","☑"]}};
...
tuesday.getRange("G9:G20").values=[["Pay the current balance, including listed pending charges"],["Pay the current balance, including listed pending charges"],["No payment needed; $0 balance and no pending charges confirmed"],["No payment needed; $0 balance and no pending charges confirmed"],["No payment needed"],["Four weekly payments remain before Oct. 10"],["No rent contribution this Tuesday because required actions exceed Wells cash"],["Move to CUTX reserve; do not pay the loan early"],["Automatic commitment; included in cash required"],["Automatic commitment; included in cash required"],["Automatic commitment; included in cash required"],["Automatic commitment; included in cash required"]];tuesday.getRange("G21").formulas=[["=IF(D21=0,\"No transfer needed; leave $1 in Wells\",\"Transfer this amount from Wealthfront to leave $1 in Wells\")"]];
...
tuesday.getRange("E9:E21").conditionalFormats.add("containsText",{text:"☐",format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});
tuesday.getRange("E9:E21").conditionalFormats.add("containsText",{text:"☑",format:{fill:mint,font:{bold:true,color:green}}});
...
dash.getRange("A2:C2").merge();dash.getRange("A2").values=[["Open this page during the import. Wells updates first; the current cash plan is built only after every required source is verified. Use Tuesday Review only after that handoff."]];dash.getRange("A2:C2").format={font:{italic:true,color:gray},wrapText:true};
sec(dash,"A4:C4","Current import — Sep 15");
dash.getRange("A5:C8").values=[["Wells available now",null,"Includes pending Kiewit payroll of $1,343.36"],["Wells import","Verified","Anchor and eight new cash actions reconciled"],["Cash plan","Built","Tuesday Review reflects all verified current actions"],["Next action","Execute plan","Use Tuesday Review checkoffs while you pay or transfer funds"]];
