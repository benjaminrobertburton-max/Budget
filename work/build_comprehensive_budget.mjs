...
// Personal-safe reports are additive contributions, not replacement balances.
...
const wealthfrontSep15InitialTransferToWells=252.61730769230803;
const wealthfrontSep15AtandtTopUp=48.59;
const wealthfrontSep15TransferToWells=wealthfrontSep15InitialTransferToWells+wealthfrontSep15AtandtTopUp;
...
budget.getRange("A7:D7").values=[["Personal-safe balance (excluded from Wells plan)","Balance",null,0]]; budget.getRange("C7").formulas=[["='Support - Account Snapshots'!F7"]];
budget.getRange("A8:D8").values=[["Usable weekly income — Wells plan",null,null,null]]; budget.getRange("D8").formulas=[["=D6"]];
...
const planRows=[
 ...
 ["Savings","Fidelity Roth IRA + PayPal savings buckets","Weekly",0,null,"$75 Roth IRA transfer plus automatic PayPal savings for cruise, tuition, and future expenses"],
...
input(budget,"C6");input(budget,"D12:D31");input(budget,"B39");budget.getRange("C7:D7").format={fill:pale,font:{color:"#000000"}};budget.getRange("D20:D22").format={fill:pale,font:{color:"#000000"}};budget.getRange("D27").format={fill:pale,font:{color:"#000000"}};
budget.getRange("C7:D8").format.numberFormat=usd;budget.getRange("D6:D8").format.numberFormat=usd;budget.getRange("D12:E34").format.numberFormat=usd;budget.getRange("B39").format.numberFormat="0.0000";budget.getRange("B40").format.numberFormat=cad;budget.getRange("C40:C41").format.numberFormat=usd;
...
const billRows=[
 ["Chase card",132.08,0,new Date("2026-09-24"),"27.49% purchases (statement)","USD","Current post-payment exposure. The $813.84 payment cleared the $786.38 pre-payment balance plus the $27.46 that posted; six pending charges total $132.08."],
...
sources.getRange("A4:E13").values=[
 ["Wells Fargo checking","Sep. 9 balance + operating history", "Sep 9, 2026", "Available Wells $1,227.78 after pending Fidelity debit; personal-safe balance was $220 after the earlier $60 and additional $160", "The earlier Jul–Aug account history establishes the $1,343.37 recurring-payroll baseline and $75 weekly Fidelity transfer. Personal-safe cash is savings-only and excluded from Wells cash-plan and spending-capacity calculations. On Sep. 15, an additional $510 brought personal-safe cash to $730."],
 ...
 ["Chase statements","Card balance, spending, subscriptions", "Jun-Aug 2026 and Sep. 15 follow-up", "Invincibles app charges: $596.43, $605.63, then $972.68 across the three statement periods; Spectrum internet, Max, dog grooming, and BudgetPetCare observed", "Sep. 15 follow-up: six pending charges total $132.08. DoorDash $20.91 and Starbucks $6.55 posted, matching the $27.46 Chase payment difference. BudgetPetCare last charged $36.45 on Jul. 29 and recurs every 3 months (next Oct. 16). Spectrum Mobile $672.29 was a one-time phone payoff funded from Wealthfront. Game/app budget cap set to $50/week. SiriusXM, NordVPN, HWBTPLUS, and Max are cancelled and excluded."],
...
savings.getRange("A12:D16").values=[["Item","Weekly amount","Weeks confirmed","Saved since tracker began"],["Fidelity Roth IRA",75,1,null],["PayPal cruise fund",85,1,null],["Del Mar tuition savings",193.36,1,null],["Future-expenses fund",50,1,null]];headers(savings,"A12:D12");savings.getRange("D13").formulas=[["=B13*C13"]];savings.getRange("D13:D16").fillDown();input(savings,"B13:C16");savings.getRange("B13:B16").format.numberFormat=usd;savings.getRange("D13:D16").format.numberFormat=usd;
...
savings.getRange("B26").formulas=[["='Support - Budget Inputs'!$E$12"]];savings.getRange("B26:B29").fillDown();savings.getRange("C26").values=[[300]];savings.getRange("C27").formulas=[["='2. Tuesday Review'!D15"]];savings.getRange("D26").formulas=[["=C26-B26"]];savings.getRange("D26:D29").fillDown();
savings.getRange("E26:F29").values=[["Wells rent fund","Actual Sep. 9 contribution; do not net against separate cash movements"],["Wells cash capacity","Calculated from verified Tuesday plan"],["Enter after review","No catch-up assumed"],["Enter after review","No catch-up assumed"]];
...
weekly.getRange("A2:F2").merge();weekly.getRange("A2").values=[["This guide refreshes at weekly closeout. During the import and Tuesday payment execution, use Start and Tuesday Review only. Savings & Debt records the current rent contribution."]];weekly.getRange("A2:F2").format={font:{italic:true,color:gray},wrapText:true};
...
weekly.getRange("A23:D23").values=[["Total weekly allocation",null,null,null]];weekly.getRange("B23").formulas=[["=SUM(B5:B22)"]];weekly.getRange("A25:D25").values=[["Usable weekly income — Wells plan",null,null,null]];weekly.getRange("B25").formulas=[["='Support - Budget Inputs'!D8"]];weekly.getRange("A26:D26").values=[["Personal-safe balance — savings only",null,null,null]];weekly.getRange("B26").formulas=[["='Support - Account Snapshots'!F7"]];weekly.getRange("A27:D27").values=[["Income used by this plan",null,null,null]];weekly.getRange("B27").formulas=[["=B25"]];weekly.getRange("A28:D28").values=[["Weekly buffer / shortfall from Wells income",null,null,null]];weekly.getRange("B28").formulas=[["=B27-B23"]];
...
const merchantRules=[
 ...
 ["Exxon","Fuel","Include","","",""],
 ["Citi","Fuel","Include","","",""],
 ["Fidelity","Fidelity Roth IRA","Transfer / verify","","",""],
...
const ledgerRows=[
 ...
 ["Wells Fargo",new Date("2026-09-10"),"PAYPAL INST XFER CREDIT REPAYMENT",148.62,"Posted","PayPal promotional payoff","Exclude","wells-2026-09-10-paypal-promo-14862","Sep. 9 payment-plan action confirmed by Wells"],
 ["Wells Fargo",new Date("2026-09-10"),"ONLINE TRANSFER TO CUTX SAV",172.26,"Posted","Car reserve / CUTX","Transfer / verify","wells-2026-09-10-cutx-17226","Sep. 9 payment-plan action confirmed by Wells"],
 ["Wells Fargo",new Date("2026-09-10"),"ONLINE TRANSFER TO UMB NA CHK 4353",300,"Posted","Rent reserve","Transfer / verify","wells-2026-09-10-rent-umb-30000","Prior circular rent method: $300.00 out, offset by the $121.51 Wealthfront return"],
 ["Wells Fargo",new Date("2026-09-10"),"INSTANT PMT FROM WEALTHFRONT BROKERAGE",-121.51,"Posted","Savings transfer","Transfer / verify","wells-2026-09-10-wealthfront-12151","Prior circular rent method: Wealthfront return; net rent funding was $178.49"],
 ["Wells Fargo",new Date("2026-09-11"),"CHASE CREDIT CRD EPAY",266.96,"Posted","Credit-card payment","Exclude","wells-2026-09-11-chase-payment-26696","Sep. 9 payment-plan action confirmed by Wells"],
 ["Wells Fargo",new Date("2026-09-11"),"CITI CARD ONLINE PAYMENT",132.09,"Posted","Credit-card payment","Exclude","wells-2026-09-11-citi-payment-13209","Sep. 9 payment-plan action confirmed by Wells"],
 ["Wells Fargo",new Date("2026-09-14"),"ZELLE TO VIRGINIA",280,"Posted","Emergency car repair","Exclude","wells-2026-09-14-zelle-virginia-28000","Emergency car repair; separate from the $471.61 McWhorter repair and excluded from the core purchase scorecard"],
...
scorecard.getRange("A2:G2").merge();scorecard.getRange("A2").values=[["Closeout analysis for new purchases against weekly category limits. Pending charges count so the week cannot look safer than it is. This is not the cash decision for today; payments and transfers do not belong here. Only source-verified ledger rows are counted."]];scorecard.getRange("A2:G2").format={font:{italic:true,color:gray},wrapText:true};
...
history.getRange("D6:F6").values=[[1227.78,300,1348.2883076923076]];history.getRange("H6").values=[[476.6607692307693]];history.getRange("G6").values=[["✓ Completed"]];history.getRange("D7:F7").values=[[1392.72,0,1692.927307692308]];history.getRange("G7").values=[["Manual actions completed — Sep 15"]];history.getRange("M5:M7").values=[["Legacy baseline: no full line-level source archive existed before the Sep. 9 audit. Do not treat this row as source-certified."],["Sep. 15 Wells capture confirmed all Sep. 9 manual actions. $300.00 was transferred to the rent fund. The separate $121.51 Wealthfront return is not netted against rent funding."],["All five Sep. 15 manual actions are user-confirmed complete. The Wealthfront top-up totals $301.21: $252.62 initially plus the $48.59 AT&T top-up. Automatic transfers remain for next-import verification."]];history.getRange("G5:G30").dataValidation={rule:{type:"list",values:["In progress","Plan created — Sep 9","Plan created — Sep 15","Manual actions completed — Sep 15","Data verified — Sep 9","Legacy — not source-audited","✓ Completed","Needs review"]}};input(history,"D7:H30");input(history,"G5:G30");input(history,"M5:M30");history.getRange("A5:C30").format.numberFormat="mmm d, yyyy";history.getRange("D5:F30").format.numberFormat=usd;history.getRange("H5:L30").format.numberFormat=usd;history.getRange("L5:L30").conditionalFormats.add("cellIs",{operator:"lessThan",formula:0,format:{fill:red,font:{bold:true,color:"#B91C1C"}}});widths(history,[["A:C",17],["D:F",19],["G:G",24],["H:L",17],["M:M",42]]);history.getRange("A1:M30").format.wrapText=true;history.getRange("A1:M30").format.font={size:12};history.getRange("A5:M30").format.rowHeight=24;history.getRange("A5:M7").format.rowHeight=42;history.showGridLines=false;history.freezePanes.freezeRows(4);
...
cash.getRange("I14").formulas=[["=IF(D14=\"\",\"Needed\",IF(COUNTIFS('Support - Ledger'!$K$5:$K$304,B14,'Support - Ledger'!$M$5:$M$304,\"Needs verification\")>0,\"Needs classification\",IF(E14<>D14,\"Missing entry\",IF(ABS(G14-F14)>0.005,\"Total mismatch\",IF(H14<>\"Yes\",\"Anchor not reached\",\"Verified\")))))"]];cash.getRange("I15").formulas=[["=IF(D15=\"\",\"Needed\",IF(COUNTIFS('Support - Ledger'!$K$5:$K$304,B15,'Support - Ledger'!$M$5:$M$304,\"Needs verification\")>0,\"Needs classification\",IF(E15<>D15,\"Missing entry\",IF(ABS(G15-F15)>0.005,\"Total mismatch\",IF(H15<>\"Yes\",\"Anchor not reached\",\"Verified\")))))"]];cash.getRange("I16").formulas=[["=IF(D16=\"\",\"Needed\",IF(COUNTIFS('Support - Ledger'!$K$5:$K$304,B16,'Support - Ledger'!$M$5:$M$304,\"Needs verification\")>0,\"Needs classification\",IF(E16<>D16,\"Missing entry\",IF(ABS(G16-F16)>0.005,\"Total mismatch\",IF(H16<>\"Yes\",\"Anchor not reached\",\"Verified\")))))"]];cash.getRange("I18").formulas=[["=IF(H18<>\"Yes\",\"Anchor not reached\",IF(E18<>D18,\"Missing entry\",IF(ABS(G18-F18)>0.005,\"Total mismatch\",\"Verified\")))"]];
cash.getRange("A11:J21").format.wrapText=true;cash.getRange("A12:J21").format.rowHeight=30;cash.getRange("F12:G18").format.numberFormat=usd;cash.getRange("I12:I21").conditionalFormats.add("containsText",{text:"Verified",format:{fill:mint,font:{bold:true,color:green}}});cash.getRange("I12:I21").conditionalFormats.add("containsText",{text:"Missing",format:{fill:red,font:{bold:true,color:"#B91C1C"}}});cash.getRange("I12:I21").conditionalFormats.add("containsText",{text:"mismatch",format:{fill:red,font:{bold:true,color:"#B91C1C"}}});cash.getRange("I12:I21").conditionalFormats.add("containsText",{text:"Needs classification",format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});cash.getRange("I12:I21").conditionalFormats.add("containsText",{text:"Needed",format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});
...
tuesday.getRange("A8:G8").values=[["Type","Payment or transfer","Basis","Amount","Status","Action","Confirmation note"]];headers(tuesday,"A8:G8");
tuesday.getRange("A9:C21").values=[["Card payment","Chase Sapphire","Actual Sep. 15 payment"],["Card payment","Citi card","Actual Sep. 15 payment"],["Card payment","Discover card","Current balance $0.00"],["Card payment","Prime Visa","Current balance $0.00"],["Card payment","Capital One","No balance due"],["Debt payment","PayPal promotional payoff","Stage 1 payoff target"],["Transfer","Rent fund — October","Only verified Wells capacity after required actions"],["Transfer","Car fund / CUTX","Actual Sep. 15 funding"],["Automatic","Fidelity Roth IRA","Weekly automatic transfer"],["Automatic","PayPal cruise savings","Weekly automatic transfer"],["Automatic","PayPal tuition savings","Weekly automatic transfer"],["Automatic","PayPal future-expenses savings","Weekly automatic transfer"],["Transfer","Wealthfront to Wells","Cover the verified cash shortfall and leave $1 in Wells"]];
tuesday.getRange("D9:D11").values=[[813.84],[106.26],[0]];tuesday.getRange("D12:D13").values=[[0],[0]];tuesday.getRange("D14").formulas=[["='Support - Promo Detail'!F12"]];tuesday.getRange("D15").formulas=[["=MAX(0,E24-SUM(D9:D14,D16:D20)-E25)"]];tuesday.getRange("D16").formulas=[["='Support - Budget Inputs'!E13"]];tuesday.getRange("D17").values=[[75]];tuesday.getRange("D18").values=[[85]];tuesday.getRange("D19").values=[[193.36]];tuesday.getRange("D20").values=[[50]];tuesday.getRange("D21").formulas=[["=MAX(0,E23+E25-E24)"]];
tuesday.getRange("E9:E21").values=[["Done"],["Done"],["Not due"],["Not due"],["Not due"],["Done"],["Not needed"],["Done"],["Automatic"],["Automatic"],["Automatic"],["Automatic"],["Done"]];
tuesday.getRange("F9:F21").values=[["Manual"],["Manual"],["Not due"],["Not due"],["Not due"],["Manual"],["Not needed"],["Manual"],["Automatic"],["Automatic"],["Automatic"],["Automatic"],["Manual"]];
tuesday.getRange("G9:G20").values=[["Paid Sep. 15; actual payment replaces the planned balance"],["Paid Sep. 15"],["No payment needed; $0 balance and no pending charges confirmed"],["No payment needed; $0 balance and no pending charges confirmed"],["No payment needed"],["Paid Sep. 15; preserve the Oct. 10 payoff schedule"],["No rent contribution this Tuesday because required actions exceed Wells cash"],["Paid Sep. 15"],["Automatic commitment; included in cash required"],["Automatic commitment; included in cash required"],["Automatic commitment; included in cash required"],["Automatic commitment; included in cash required"]];tuesday.getRange("G21").values=[["Transferred Sep. 15; user confirmed"]];
tuesday.getRange("D9:D21").format={fill:pale,font:{color:"#000000"}};
tuesday.getRange("E9:E21").format={fill:"#E8EEF5",font:{color:gray,bold:true},horizontalAlignment:"center"};tuesday.getRange("E9").format={fill:yellow,font:{color:"#7C5E10",bold:true},horizontalAlignment:"center"};tuesday.getRange("E10").format={fill:yellow,font:{color:"#7C5E10",bold:true},horizontalAlignment:"center"};tuesday.getRange("E14").format={fill:yellow,font:{color:"#7C5E10",bold:true},horizontalAlignment:"center"};tuesday.getRange("E16").format={fill:yellow,font:{color:"#7C5E10",bold:true},horizontalAlignment:"center"};tuesday.getRange("E21").format={fill:yellow,font:{color:"#7C5E10",bold:true},horizontalAlignment:"center"};
tuesday.getRange("F9:F21").format={fill:pale,font:{color:gray},horizontalAlignment:"center"};
tuesday.getRange("E9:E21").conditionalFormats.add("containsText",{text:"Pay today",format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});
tuesday.getRange("E9:E21").conditionalFormats.add("containsText",{text:"Done",format:{fill:mint,font:{bold:true,color:green}}});
tuesday.getRange("A22:C22").merge();tuesday.getRange("A22").values=[["Planned subtotal (cash actions)"]];tuesday.getRange("A22:C22").format={fill:pale,font:{bold:true,color:teal}};tuesday.getRange("D22").formulas=[["=SUM(D9:D20)"]];tuesday.getRange("D22").format={fill:pale,font:{bold:true,color:teal},numberFormat:usd,borders:{preset:"doubleBottom",style:"medium",color:teal}};
tuesday.getRange("A23:D23").merge();tuesday.getRange("A23").values=[["Cash required for payments and transfers"]];tuesday.getRange("A23:D23").format={fill:mint,font:{bold:true,color:teal}};tuesday.getRange("E23").formulas=[["=SUM(D9:D20)"]];
tuesday.getRange("A24:D24").merge();tuesday.getRange("A24").values=[["Wells available from verified import"]];tuesday.getRange("A24:D24").format={fill:mint,font:{bold:true,color:teal}};tuesday.getRange("E24").formulas=[["='1. Start'!B5"]];
tuesday.getRange("A25:D25").merge();tuesday.getRange("A25").values=[["Minimum Wells balance after all actions"]];tuesday.getRange("A25:D25").format={fill:yellow,font:{bold:true,color:navy}};tuesday.getRange("E25").values=[[1]];
tuesday.getRange("A26:D26").merge();tuesday.getRange("A26").values=[["Move from Wealthfront to Wells if needed"]];tuesday.getRange("A26:D26").format={fill:mint,font:{bold:true,color:teal}};tuesday.getRange("E26").formulas=[["=D21"]];
tuesday.getRange("E23:E26").format={fill:pale,font:{bold:true,size:14},numberFormat:usd,borders:{preset:"outside",style:"medium",color:"#A7C9C8"}};tuesday.getRange("D9:D21").format.numberFormat=usd;

tuesday.getRange("A1:G26").format.wrapText=true;tuesday.getRange("A9:G21").format.rowHeight=25;widths(tuesday,[["A:A",18],["B:B",28],["C:C",21],["D:D",18],["E:E",19],["F:F",13],["G:G",41]]);tuesday.getRange("A1:G26").format.font={size:12};tuesday.showGridLines=false;tuesday.freezePanes.freezeRows(8);

// Corrective cash-plan overlay: include the user-confirmed AT&T Wells payment in required cash.
...
title(dash,"A1:C1","Start — Weekly Import",operations);
dash.getRange("A2:C2").merge();dash.getRange("A2").values=[["Open this page only during the weekly import. Wells updates first; the cash plan is built only after every required source is verified. During import and payment execution, updates stay limited to Start, Tuesday Review, and the transaction ledger."]];dash.getRange("A2:C2").format={font:{italic:true,color:gray},wrapText:true};
sec(dash,"A4:C4","Current import — Sep 15");
dash.getRange("A5:C8").values=[["Wells available now",null,"Includes pending Kiewit payroll of $1,343.36"],["Wells import","Verified","Anchor and eight new cash actions reconciled"],["Cash plan","Manual actions complete","Automatic transfers await next import"],["Next action","Verify automatics","At the next weekly import, confirm automatic transfers from account activity"]];
dash.getRange("B5").formulas=[["='Support - Account Snapshots'!F5"]];
dash.getRange("A5:C8").format={fill:pale,borders:{preset:"inside",style:"thin",color:"#D9E5E7"},wrapText:true};dash.getRange("B5").format={fill:"#FFFFFF",font:{bold:true,size:14},numberFormat:usd};dash.getRange("B6:B8").format={fill:"#FFFFFF",font:{bold:true,size:12}};dash.getRange("A6:C8").format={fill:mint,font:{bold:true}};
sec(dash,"A10:C10","Input readiness before the next review");
dash.getRange("A11:C11").values=[["Check","Open items","Next input"]];headers(dash,"A11:C11");
dash.getRange("A12:C17").values=[
 ["Source-control exceptions",null,"Resolve missing entry, total mismatch, or anchor issue in Account Snapshots"],
 ["Ledger rows needing audit fields",null,"Add source archive, visible section, and verification status for each new transaction"],
 ["Debt / bill statement or due dates blank",null,"Update yellow due-date cells when the account screen or statement shows them"],
 ["Debt rows marked for confirmation",null,"Refresh balances, minimums, or terms that are still labeled confirm"],
 ["Pending transactions to classify",null,"Capture every pending item and keep unclear amounts flagged"],
 ["Next weekly import",null,"Update the current review date, anchors, and source-control rows"]
 ];
dash.getRange("B12").formulas=[["=COUNTIF('Support - Account Snapshots'!I12:I21,\"Missing entry\")+COUNTIF('Support - Account Snapshots'!I12:I21,\"Total mismatch\")+COUNTIF('Support - Account Snapshots'!I12:I21,\"Anchor not reached\")+COUNTIF('Support - Account Snapshots'!I12:I21,\"Source check needed\")"]];
dash.getRange("B13").formulas=[["=SUMPRODUCT(--('Support - Ledger'!C5:C304<>\"\"),--(('Support - Ledger'!K5:K304=\"\")+('Support - Ledger'!L5:L304=\"\")+('Support - Ledger'!M5:M304=\"\")>0))"]];
dash.getRange("B14").formulas=[["=COUNTBLANK('Support - Debt Detail'!D5:D15)"]];
dash.getRange("B15").formulas=[["=COUNTIF('Support - Debt Detail'!G5:G15,\"*confirm*\")"]];
dash.getRange("B16").formulas=[["=COUNTIF('Support - Ledger'!E5:E304,\"Pending\")"]];
dash.getRange("B17").values=[[1]];
dash.getRange("A11:C17").format={fill:pale,borders:{preset:"inside",style:"thin",color:"#D9E5E7"},wrapText:true,verticalAlignment:"center"};dash.getRange("B12:B17").format={fill:"#FFFFFF",font:{bold:true},horizontalAlignment:"center",numberFormat:"0"};dash.getRange("A12:A17").format.font={bold:true};dash.getRange("B12:B16").conditionalFormats.add("cellIs",{operator:"greaterThan",formula:0,format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});dash.getRange("B12:B16").conditionalFormats.add("cellIs",{operator:"equal",formula:0,format:{fill:mint,font:{bold:true,color:green}}});dash.getRange("A12:C17").format.rowHeight=32;
...
dash.getRange("E6").formulas=[["='Support - Account Snapshots'!I12"]];dash.getRange("E7").formulas=[["='Support - Account Snapshots'!I13"]];dash.getRange("E8").formulas=[["=IF(AND('Support - Account Snapshots'!I14=\"Verified\",'Support - Account Snapshots'!I15=\"Verified\"),\"Verified\",\"Needed\")"]];dash.getRange("E9").formulas=[["='Support - Account Snapshots'!I16"]];dash.getRange("E10").formulas=[["='Support - Account Snapshots'!I17"]];dash.getRange("E11").formulas=[["='Support - Account Snapshots'!I18"]];dash.getRange("E12").formulas=[["='Support - Account Snapshots'!I19"]];dash.getRange("E13").formulas=[["='Support - Account Snapshots'!I20"]];
...
dash.getRange("E6:E13").conditionalFormats.add("containsText",{text:"Needs classification",format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});dash.getRange("E6:E13").conditionalFormats.add("containsText",{text:"Needed",format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});dash.getRange("E6:E13").conditionalFormats.add("containsText",{text:"Source check",format:{fill:red,font:{bold:true,color:"#B91C1C"}}});
...
dash.getRange("A1:H17").format.wrapText=true;dash.getRange("A1:H17").format.font={size:12};dash.getRange("A5:C8").format.rowHeight=29;dash.getRange("A12:C17").format.rowHeight=46;dash.getRange("E6:H14").format.rowHeight=30;dash.getRange("E16:H17").format.rowHeight=28;widths(dash,[["A:A",29],["B:B",22],["C:C",36],["D:D",3],["E:E",12],["F:F",19],["G:G",33],["H:H",21]]);dash.showGridLines=false;dash.freezePanes.freezeRows(4);
...
polish(dash,"A1:H17","A1:C1","A2:C2",["A4:C4","E1:H1","E4:H4"]);dash.getRange("E5:H5").format.font={name:"Arial",size:10,bold:true,color:"#243447"};dash.getRange("A5:C8").format.rowHeight=28;dash.getRange("A8:C8").format.rowHeight=40;dash.getRange("A12:C17").format.rowHeight=46;dash.getRange("E6:H14").format.rowHeight=28;dash.getRange("E16:H17").format.rowHeight=25;dash.getRange("B5:B8").format.font={name:"Arial",size:14,bold:true,color:"#172B4D"};
polish(tuesday,"A1:G26","A1:G1","A2:G2",["A4:G4"]);tuesday.getRange("A5:G6").format.rowHeight=24;tuesday.getRange("A8:G8").format.rowHeight=23;tuesday.getRange("A9:G21").format.rowHeight=28;tuesday.getRange("D9:D21").format.font={name:"Arial",size:11,bold:true,color:"#243447"};tuesday.getRange("F9:F21").format.horizontalAlignment="center";tuesday.getRange("E23:E26").format.font={name:"Arial",size:13,bold:true,color:"#172B4D"};
polish(dash,"A1:H17","A1:C1","A2:C2",["A4:C4","E1:H1","E4:H4"]);dash.getRange("E5:H5").format.font={name:"Arial",size:10,bold:true,color:"#243447"};dash.getRange("A5:C8").format.rowHeight=28;dash.getRange("A8:C8").format.rowHeight=40;dash.getRange("A12:C17").format.rowHeight=46;dash.getRange("E6:H14").format.rowHeight=28;dash.getRange("E16:H17").format.rowHeight=25;dash.getRange("B5:B8").format.font={name:"Arial",size:14,bold:true,color:"#172B4D"};
