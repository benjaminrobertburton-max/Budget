...
title(tuesday,"A1:G1","Tuesday Review — Sep 9 Completed",operations);
tuesday.getRange("A2:G2").merge();tuesday.getRange("A2").values=[["This completed plan is retained as history. For the next Tuesday, use the card-payment queue below only after Start confirms the current account imports."]];tuesday.getRange("A2:G2").format={font:{italic:true,color:gray},wrapText:true};
...
tuesday.getRange("A6").formulas=[["=COUNTIF('1. Start'!E6:E13,\"Verified\")&\" of \"&(COUNTA('1. Start'!F6:F14)-COUNTIF('1. Start'!E6:E14,\"Not due\"))&\" source sets verified\""]];tuesday.getRange("C6").formulas=[["=COUNTIFS(F9:F19,\"Manual\",E9:E19,\"☑ Done\")&\" / \"&COUNTIF(F9:F19,\"Manual\")&\" manual payments complete\""]];
tuesday.getRange("A6:G6").format={fill:pale,font:{bold:true},wrapText:true};
tuesday.getRange("A8:G8").values=[["Type","Payment or transfer","Basis","Planned amount","Check off","Action","Confirmation note"]];headers(tuesday,"A8:G8");
tuesday.getRange("A9:C19").values=[["Card payment","Chase Sapphire","Current Sep. 9 balance"],["Card payment","Citi card","Current Sep. 9 balance"],["Card payment","Prime Visa","No balance due"],["Debt payment","PayPal promotional payoff","Stage 1 payoff target"],["Transfer","Rent fund — October","Wells capacity after required actions; capped at normal plan"],["Transfer","Car fund / CUTX","Weekly reserve toward Sep. 27 due date"],["Automatic","Fidelity Roth IRA","Pending; already in Wells available balance"],["Automatic","PayPal cruise savings","Flexible savings contribution"],["Automatic","PayPal tuition savings","Weekly scheduled transfer"],["Automatic","PayPal future-expenses savings","Flexible savings contribution"],["Transfer","Wealthfront to Wells","Cover a remaining cash shortfall"]];
tuesday.getRange("D9:D10").values=[[266.96],[132.09]];tuesday.getRange("D11").values=[[0]];tuesday.getRange("D12").formulas=[["='Support - Promo Detail'!F12"]];tuesday.getRange("D13").formulas=[["=MIN('5. Savings & Debt'!$B$26,MAX(0,$E$22-SUM($D$9:$D$12,$D$14,$D$16:$D$18)-$E$23))"]];tuesday.getRange("D14").formulas=[["='Support - Budget Inputs'!E13"]];tuesday.getRange("D15").values=[[75]];tuesday.getRange("D16").values=[[85]];tuesday.getRange("D17").values=[[193.36]];tuesday.getRange("D18").values=[[50]];tuesday.getRange("D19").formulas=[["=MAX(0,E21+E23-E22)"]];
tuesday.getRange("E9:E19").values=[["☑ Done"],["☑ Done"],["— Not due"],["☑ Done"],["☑ Done"],["☑ Done"],["Auto — no click"],["Auto — no click"],["Auto — no click"],["Auto — no click"],["— Not needed"]];
tuesday.getRange("E9:E19").dataValidation={rule:{type:"list",values:["☐ Pay now","☑ Done","Partial","— Not due","Auto — no click","— Not needed"]}};
tuesday.getRange("F9:F19").values=[["Manual"],["Manual"],["Not due"],["Manual"],["Manual"],["Manual"],["Automatic"],["Automatic"],["Automatic"],["Automatic"],["Not needed"]];
tuesday.getRange("G9:G18").values=[["Pay the current balance if maintaining the weekly payoff routine"],["Pay the current balance if maintaining the weekly payoff routine"],["No payment needed"],["Five weekly payments remain before Oct. 10"],["Move only this calculated amount to rent reserve; it records today’s actual funding"],["Move to CUTX reserve; do not pay the loan early"],["Do not count again in cash required"],["Automatic commitment; included in cash required"],["Automatic commitment; included in cash required"],["Automatic commitment; included in cash required"]];tuesday.getRange("G19").formulas=[["=IF(D19=0,\"No transfer needed after net rent funding\",\"Transfer this amount from Wealthfront so Wells retains $1 after all actions\")"]];
tuesday.getRange("D9:D19").format={fill:pale,font:{color:"#000000"}};
tuesday.getRange("E9:E19").format={fill:yellow,font:{color:"#0000FF",bold:true},horizontalAlignment:"center"};
tuesday.getRange("F9:F19").format={fill:pale,font:{color:gray},horizontalAlignment:"center"};
tuesday.getRange("E9:E19").conditionalFormats.add("containsText",{text:"☐ Pay now",format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});
tuesday.getRange("E9:E19").conditionalFormats.add("containsText",{text:"☑ Done",format:{fill:mint,font:{bold:true,color:green}}});
tuesday.getRange("E9:E19").conditionalFormats.add("containsText",{text:"Auto",format:{fill:"#E8EEF5",font:{bold:true,color:gray}}});
tuesday.getRange("E9:E19").conditionalFormats.add("containsText",{text:"Not",format:{fill:"#E8EEF5",font:{color:gray}}});
...
tuesday.getRange("E21:E24").format={fill:pale,font:{bold:true,size:14},numberFormat:usd,borders:{preset:"outside",style:"medium",color:"#A7C9C8"}};tuesday.getRange("D9:D19").format.numberFormat=usd;

// The next-cycle queue is source-linked so a tracked card cannot disappear from the payment workflow.
sec(tuesday,"A26:G26","Next card-payment queue — Sep 15");
tuesday.getRange("A27:G27").values=[["Account","Import status","Posted balance","Pending exposure","Pay today","Next action","Check off"]];headers(tuesday,"A27:G27");
tuesday.getRange("A28:A33").values=[["Chase Sapphire"],["Citi AAdvantage"],["Discover"],["Prime Visa"],["Capital One"],["PayPal Credit"]];
tuesday.getRange("B28:B33").formulas=[["='1. Start'!E8"],["='1. Start'!E9"],["='1. Start'!E11"],["='1. Start'!E10"],["='1. Start'!E12"],["='1. Start'!E13"]];
tuesday.getRange("C28:C33").formulas=[["='Support - Debt Detail'!B5"],["='Support - Debt Detail'!B6"],["='Support - Debt Detail'!B7"],["='Support - Debt Detail'!B9"],["='Support - Debt Detail'!B8"],["='Support - Debt Detail'!B10"]];
tuesday.getRange("D28:D33").formulas=[
 ["=SUMIFS('Support - Ledger'!$D$5:$D$304,'Support - Ledger'!$A$5:$A$304,\"Chase\",'Support - Ledger'!$E$5:$E$304,\"Pending\",'Support - Ledger'!$K$5:$K$304,\"Chase pending — Sep 15\")"],
 ["=SUMIFS('Support - Ledger'!$D$5:$D$304,'Support - Ledger'!$A$5:$A$304,\"Citi\",'Support - Ledger'!$E$5:$E$304,\"Pending\",'Support - Ledger'!$K$5:$K$304,\"Citi activity — Sep 15\")"],
 ["=SUMIFS('Support - Ledger'!$D$5:$D$304,'Support - Ledger'!$A$5:$A$304,\"Discover\",'Support - Ledger'!$E$5:$E$304,\"Pending\",'Support - Ledger'!$K$5:$K$304,\"Discover balance / activity — Sep 15\")"],
 ["=SUMIFS('Support - Ledger'!$D$5:$D$304,'Support - Ledger'!$A$5:$A$304,\"Prime Visa\",'Support - Ledger'!$E$5:$E$304,\"Pending\",'Support - Ledger'!$K$5:$K$304,\"Prime Visa activity — Sep 15\")"],
 ["=SUMIFS('Support - Ledger'!$D$5:$D$304,'Support - Ledger'!$A$5:$A$304,\"Capital One\",'Support - Ledger'!$E$5:$E$304,\"Pending\",'Support - Ledger'!$K$5:$K$304,\"Capital One balance / activity — Sep 15\")"],
 ["=SUMIFS('Support - Ledger'!$D$5:$D$304,'Support - Ledger'!$A$5:$A$304,\"PayPal Credit\",'Support - Ledger'!$E$5:$E$304,\"Pending\",'Support - Ledger'!$K$5:$K$304,\"PayPal balance + promos — Sep 15\")"]
];
tuesday.getRange("E28:E32").formulas=[["=IF(B28<>\"Verified\",\"\",C28)"]];tuesday.getRange("E28:E32").fillDown();tuesday.getRange("E33").formulas=[["=IF(B33<>\"Verified\",\"\",'Support - Promo Detail'!F12)"]];
tuesday.getRange("F28:F32").formulas=[["=IF(B28<>\"Verified\",\"Await import\",IF(E28>0,\"Pay posted balance\",IF(D28>0,\"Reserve pending; pay after posting\",\"No payment needed\")))"]];tuesday.getRange("F28:F32").fillDown();tuesday.getRange("F33").formulas=[["=IF(B33<>\"Verified\",\"Await import\",IF(E33>0,\"Pay weekly promo target\",\"No payment needed\"))"]];
tuesday.getRange("G28:G33").values=[["Await import"],["Needs classification"],["Await import"],["Await import"],["Await import"],["Await import"]];
tuesday.getRange("G28:G33").dataValidation={rule:{type:"list",values:["☐ Pay now","☑ Done","Partial","— Pending","— Not needed","Await import","Needs classification"]}};
tuesday.getRange("A28:G33").format={fill:pale,borders:{preset:"inside",style:"thin",color:"#D9E5E7"},wrapText:true,verticalAlignment:"center"};
tuesday.getRange("A28:A33").format.font={bold:true};tuesday.getRange("B28:B33").format={fill:"#FFFFFF",font:{bold:true},horizontalAlignment:"center"};tuesday.getRange("C28:E33").format={fill:"#FFFFFF",font:{bold:true},numberFormat:usd};tuesday.getRange("F28:F33").format={fill:"#FFFFFF",font:{color:gray}};tuesday.getRange("G28:G33").format={fill:yellow,font:{color:"#0000FF",bold:true},horizontalAlignment:"center"};
tuesday.getRange("B28:B33").conditionalFormats.add("containsText",{text:"Verified",format:{fill:mint,font:{bold:true,color:green}}});tuesday.getRange("B28:B33").conditionalFormats.add("containsText",{text:"Needed",format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});tuesday.getRange("B28:B33").conditionalFormats.add("containsText",{text:"classification",format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});
tuesday.getRange("G28:G33").conditionalFormats.add("containsText",{text:"☑ Done",format:{fill:mint,font:{bold:true,color:green}}});tuesday.getRange("G28:G33").conditionalFormats.add("containsText",{text:"Await",format:{fill:"#E8EEF5",font:{color:gray}}});tuesday.getRange("G28:G33").conditionalFormats.add("containsText",{text:"classification",format:{fill:yellow,font:{bold:true,color:"#7C5E10"}}});
tuesday.getRange("A35:D35").merge();tuesday.getRange("A35").values=[["Next-queue manual payments completed"]];tuesday.getRange("A35:D35").format={fill:mint,font:{bold:true,color:teal}};tuesday.getRange("E35:G35").merge();tuesday.getRange("E35").formulas=[["=COUNTIF(G28:G33,\"☑ Done\")&\" / \"&(COUNTIF(F28:F32,\"Pay posted balance\")+COUNTIF(F33,\"Pay weekly promo target\"))&\" complete\""]];tuesday.getRange("E35:G35").format={fill:mint,font:{bold:true,color:teal},horizontalAlignment:"center"};
tuesday.getRange("A1:G35").format.wrapText=true;tuesday.getRange("A9:G19").format.rowHeight=25;tuesday.getRange("A28:G33").format.rowHeight=30;widths(tuesday,[["A:A",18],["B:B",28],["C:C",21],["D:D",18],["E:E",18],["F:F",31],["G:G",22]]);tuesday.getRange("A1:G35").format.font={size:12};tuesday.showGridLines=false;tuesday.freezePanes.freezeRows(8);
...
dash.getRange("A5:C8").values=[["Wells available now",null,"Includes pending Kiewit payroll of $1,343.36"],["Wells import","Verified","Anchor and eight new cash actions reconciled"],["Cash plan","Waiting","Build after the remaining source imports are verified"],["Next action","Continue","Import the remaining account sources"]];
dash.getRange("B5").formulas=[["='Support - Account Snapshots'!F5"]];
dash.getRange("A5:C8").format={fill:pale,borders:{preset:"inside",style:"thin",color:"#D9E5E7"},wrapText:true};dash.getRange("B5").format={fill:"#FFFFFF",font:{bold:true,size:14},numberFormat:usd};dash.getRange("B6:B8").format={fill:"#FFFFFF",font:{bold:true,size:12}};dash.getRange("A6:C6").format={fill:mint,font:{bold:true}};dash.getRange("A7:C7").format={fill:yellow,font:{bold:true}};dash.getRange("A8:C8").format={fill:mint,font:{bold:true}};
...
polish(tuesday,"A1:G35","A1:G1","A2:G2",["A4:G4","A26:G26"]);tuesday.getRange("A5:G6").format.rowHeight=24;tuesday.getRange("A8:G8").format.rowHeight=23;tuesday.getRange("A9:G19").format.rowHeight=28;tuesday.getRange("A27:G27").format.rowHeight=25;tuesday.getRange("A28:G33").format.rowHeight=32;tuesday.getRange("D9:D19").format.font={name:"Arial",size:11,bold:true,color:"#243447"};tuesday.getRange("F9:F19").format.horizontalAlignment="center";tuesday.getRange("E21:E24").format.font={name:"Arial",size:13,bold:true,color:"#172B4D"};
polish(dash,"A1:H17","A1:C1","A2:C2",["A4:C4","E1:H1","E4:H4"]);dash.getRange("E5:H5").format.font={name:"Arial",size:10,bold:true,color:"#243447"};dash.getRange("A5:C8").format.rowHeight=28;dash.getRange("A8:C8").format.rowHeight=40;dash.getRange("A12:C17").format.rowHeight=46;dash.getRange("E6:H14").format.rowHeight=28;dash.getRange("E16:H17").format.rowHeight=25;dash.getRange("B5:B8").format.font={name:"Arial",size:14,bold:true,color:"#172B4D"};
...
const tuesdayCheck=await wb.inspect({kind:"table",range:"2. Tuesday Review!A1:G35",include:"values,formulas",tableMaxRows:35,tableMaxCols:7});console.log(tuesdayCheck.ndjson);
