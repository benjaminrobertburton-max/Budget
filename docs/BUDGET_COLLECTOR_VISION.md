...
Implementation has begun on `codex/budget-collector`. `collector/README.md` documents the offline foundation, Windows user-bound encrypted storage, disposable Chrome demonstrations, tests, and remaining production gates. Version 0.5 adds separately gated visible Wells pilot controls; opening a bank requires the user's explicit acknowledgement/action and login remains manual. Version 0.5.1 adds a narrow, publicly reviewed GET visual-asset exception after the first attended attempt reached an unstyled summary; see `docs/WELLS_ASSET_REVIEW.md`. Its inspection exports only text-free page structure, never financial evidence or proof of account coverage. The fictional transport remains loopback-only; no bank readers are installed. Its fictional-data candidate can be stored in disposable encrypted test records but is not a verified import and cannot update a workbook. Work-machine tests never initialize the real private financial store; that belongs on the home machine. The existing financial workbook and builder remain unchanged.
...
- `collector/src/disposable-run.mjs` implements the owned-run lifecycle, encrypted evidence helper, close hooks, leftover blocking, and verified removal. Both fictional storage and real-Chrome/fictional-page demonstrations exercise it. Version 0.4 adds an explicit stopped-test recovery command, tested with a forced collector exit, and a text-free structural inspection boundary. Recovery checks directory ownership and Windows process ancestry and never kills Chrome; uncertain, damaged, or unsupported states still block. Version 0.5 adds separately gated pilot controls, a reviewed destination restriction including redirect checks, and bounded stop/cleanup. Real Wells compatibility, bank readers/evidence extraction and real approval detection remain unverified/unimplemented. Structural inspection is not account verification; do not bypass a blocked login component.
