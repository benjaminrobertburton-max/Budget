...
- `collector/src/disposable-run.mjs` implements the owned-run lifecycle, encrypted evidence helper, close hooks, leftover blocking, and verified removal. The fictional storage demonstration exercises it. Real-browser shutdown/cancellation and read-only adapters must still be integrated and tested before live collection is enabled.
